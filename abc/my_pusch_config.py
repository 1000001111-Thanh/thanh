...
from sionna.phy.nr import PUSCHConfig, CarrierConfig, PUSCHDMRSConfig, TBConfig, PUSCHPilotPattern, TBEncoder,LayerMapper, LayerDemapper, TBDecoder, PUSCHLSChannelEstimator
from sionna.phy.channel import AWGN, OFDMChannel, gen_single_sector_topology as gen_topology
from sionna.phy.ofdm import LinearDetector, ResourceGrid, ResourceGridMapper
...
class MyPUSCHConfig(PUSCHConfig):
    def __init__(self, my_config: MyConfig, slot_number: int, frame_number=0):
        assert my_config.Ue[0].NLayers == 1
        self._my_config = my_config
        super().__init__(
            carrier_config=CarrierConfig(
                n_cell_id=self._my_config.Sys.NCellId,
                cyclic_prefix="normal" if ~self._my_config.Sys.CpType else "extended",
                subcarrier_spacing=15*(2**self._my_config.Sys.Numerology),
                n_size_grid=self._my_config.Sys.BwpNRb,
                n_start_grid=self._my_config.Sys.BwpRbOffset,
                slot_number=slot_number,
                frame_number=frame_number
            ),
            pusch_dmrs_config=PUSCHDMRSConfig(
                config_type=self._my_config.Ue[0].DmrsConfigurationType + 1,
                length=self._my_config.Ue[0].DmrsDuration,
                additional_position=self._my_config.Ue[0].DmrsAdditionalPosition,
                dmrs_port_set=self._my_config.Ue[0].DmrsPortSetIdx,
                n_id=[self._my_config.Ue[0].NnScIdId,self._my_config.Ue[0].NnScIdId],
                n_scid=self._my_config.Ue[0].nScId,
                num_cdm_groups_without_data=self._my_config.Ue[0].NumDmrsCdmGroupsWithoutData,
                type_a_position=self._my_config.Ue[0].DmrsTypeAPosition
            ),
            tb_config=TBConfig(
                channel_type='PUSCH',
                n_id=self._my_config.Ue[0].nId,
                mcs_table=self._my_config.Ue[0].McsTable + 1,
                mcs_index=self._my_config.Ue[0].Mcs
            ),
            mapping_type='A' if ~self._my_config.Ue[0].PuschMappingType else 'B',
            n_size_bwp=self._my_config.Sys.BwpNRb,
            n_start_bwp=self._my_config.Sys.BwpRbOffset,
            num_layers=self._my_config.Ue[0].NLayers,
            num_antenna_ports=len(self._my_config.Ue[0].DmrsPortSetIdx),
            precoding='non-codebook' if ~self._my_config.Ue[0].CodeBookBased else 'codebook',
            tpmi=self._my_config.Ue[0].Tpmi,
            transform_precoding=False if ~self._my_config.Ue[0].TransformPrecoding else True,
            n_rnti=self._my_config.Ue[0].Rnti,
            symbol_allocation=[self._my_config.Ue[0].FirstSymb, self._my_config.Ue[0].NPuschSymbAll]
        )
    ...
    def first_resource_block(self):
        ...
        """
        return self._my_config.Ue[0].FirstPrb
    ...
    def num_resource_blocks(self):
        ...
        int, read-only : Number of allocated resource blocks for the
            PUSCH transmissions.
        """
        return self._my_config.Ue[0].NPrb
            ...
            for esno_db in esno_dbs:
                ...
                for n,slot in enumerate(slots):
                    ...
                    pusch_records.append(PuschRecord(
                                        ...
                                        nCpType=simulator.pusch_config._my_config.Sys.CpType,
                                        nULType=0,
                                        nMcsTable=simulator.pusch_config.tb.mcs_table - 1,
                                        nMCS=simulator.pusch_config.tb.mcs_index,
                                        nTransPrecode=simulator.pusch_config._my_config.Ue[0].TransformPrecoding,
                                        nTransmissionScheme=simulator.pusch_config._my_config.Ue[0].CodeBookBased,
                                        ...
                                        nNrOfAntennaPorts=simulator.pusch_config._my_config.Sys.NRxAnt,
                                        nVRBtoPRB=0,
                                        nPMI=simulator.pusch_config._my_config.Ue[0].Tpmi,
                                        ...
                                        nRV=simulator.pusch_config._my_config.Sys.rvSeq,
                                        nHARQID=n % 16,
                                        nNDI=1,
                                        nMappingType=simulator.pusch_config._my_config.Ue[0].PuschMappingType,
                                        nDMRSConfigType=simulator.pusch_config._my_config.Ue[0].DmrsConfigurationType,
                                        ...
                                        nPTRSPresent=simulator.pusch_config._my_config.Ue[0].Ptrs,
                                        nAck=simulator.pusch_config._my_config.Ue[0].OAck,
                                        nAlphaScaling=simulator.pusch_config._my_config.Ue[0].ScalingFactor,
                                        nBetaOffsetACKIndex=simulator.pusch_config._my_config.Ue[0].IHarqAckOffset,
                                        nCsiPart1=simulator.pusch_config._my_config.Ue[0].OCsi1,
                                        nBetaOffsetCsiPart1Index=simulator.pusch_config._my_config.Ue[0].ICsi1Offset,
                                        nCsiPart2=simulator.pusch_config._my_config.Ue[0].OCsi2,
                                        nBetaOffsetCsiPart2Index=simulator.pusch_config._my_config.Ue[0].ICsi2Offset,
                                        nTpPi2BPSK=simulator.pusch_config._my_config.Ue[0].TpPi2Bpsk,
                                        nTPPuschID=simulator.pusch_config._my_config.Ue[0].NRsId,
                                        nRxRUIdx=np.arange(0, simulator.pusch_config._my_config.Sys.NRxAnt),
                                        nUE=1,
                                        nPduIdx=[0],
...
def setCfgReq(puschCfg: MyPUSCHConfig, slots, dir):
    bandwidth = puschCfg._my_config.Sys.BandWidth
    fft_size = 4096
...
<Ul_Config_Req>
	...
	<UL_SCH_PDU0>
		...
		<nCpType>{puschCfg._my_config.Sys.CpType}</nCpType>
		<nULType>{0}</nULType>
		<nMcsTable>{puschCfg.tb.mcs_table - 1}</nMcsTable>
		<nMCS>{puschCfg.tb.mcs_index}</nMCS>
		<nTransPrecode>{puschCfg._my_config.Ue[0].TransformPrecoding}</nTransPrecode>
		<nTransmissionScheme>{puschCfg._my_config.Ue[0].CodeBookBased}</nTransmissionScheme>
		...
		<nNrOfAntennaPorts>{puschCfg._my_config.Sys.NRxAnt}</nNrOfAntennaPorts>
		<nVRBtoPRB>{0}</nVRBtoPRB>
		<nPMI>{puschCfg._my_config.Ue[0].Tpmi}</nPMI>
		...
		<nRV>{puschCfg._my_config.Sys.rvSeq}</nRV>
		<nHARQID>{harqIdx}</nHARQID>
		<nNDI>{1}</nNDI>
		<nMappingType>{puschCfg._my_config.Ue[0].PuschMappingType}</nMappingType>
		<nDMRSConfigType>{puschCfg._my_config.Ue[0].DmrsConfigurationType}</nDMRSConfigType>
		...
		<nPTRSPresent>{puschCfg._my_config.Ue[0].Ptrs}</nPTRSPresent>
		<nAck>{puschCfg._my_config.Ue[0].OAck}</nAck>
		<nAlphaScaling>{puschCfg._my_config.Ue[0].ScalingFactor}</nAlphaScaling>
		<nBetaOffsetACKIndex>{puschCfg._my_config.Ue[0].IHarqAckOffset}</nBetaOffsetACKIndex>
		<nCsiPart1>{puschCfg._my_config.Ue[0].OCsi1}</nCsiPart1>
		<nBetaOffsetCsiPart1Index>{puschCfg._my_config.Ue[0].ICsi1Offset}</nBetaOffsetCsiPart1Index>
		<nCsiPart2>{puschCfg._my_config.Ue[0].OCsi2}</nCsiPart2>
		<nBetaOffsetCsiPart2Index>{puschCfg._my_config.Ue[0].ICsi2Offset}</nBetaOffsetCsiPart2Index>
		<nTpPi2BPSK>{puschCfg._my_config.Ue[0].TpPi2Bpsk}</nTpPi2BPSK>
		<nTPPuschID>{puschCfg._my_config.Ue[0].NRsId}</nTPPuschID>
